    ...
    <div className="max-w-md mx-auto h-[100dvh] bg-[#0e0e10] flex flex-col relative overflow-hidden select-none">
      ...
      {selectedRecordIds.length > 0 && (
        <header className="p-4 flex items-center justify-between sticky top-0 z-[60] bg-blue-600 safe-top animate-in fade-in slide-in-from-top duration-200">
          <div className="flex items-center gap-4">
            ...
            <span className="text-xl font-semibold text-white">{selectedRecordIds.length} Selected</span>
          </div>
      ...
      <header className="p-4 flex items-center justify-between sticky top-0 z-30 bg-[#0e0e10]/90 backdrop-blur-md safe-top border-b border-zinc-900/50">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold tracking-tight text-white uppercase">ZenWallet</h1>
        </div>
      ...
      <main className="flex-1 overflow-hidden">
        {activeView === 'dashboard' && (
          <div className="px-2 py-4 space-y-6 pb-40 overflow-y-auto h-full no-scrollbar animate-in fade-in duration-500">
            {/* Accounts section */}
            <div className="flex items-center justify-between px-2">
              <h2 className="text-xs font-semibold text-zinc-500 uppercase tracking-[0.2em]">Accounts</h2>
            ...
            <div className="grid grid-cols-3 gap-2 px-1">
              ...
              <button onClick={() => { setIsAddingAccount(true); pushNav(); }} className="flex flex-col items-center justify-center p-2 rounded-2xl border border-dashed border-zinc-800 h-[84px] text-zinc-500 gap-1 bg-[#1e1e1e]/20"><Plus className="w-4 h-4" /><span className="text-[9px] font-semibold uppercase tracking-wider">Add</span></button>
            </div>

            {/* Last Records Card */}
            <div className="bg-[#1e1e1e] rounded-[1.5rem] border border-zinc-800/80 shadow-xl overflow-hidden">
               <div className="px-4 py-4 flex items-center justify-between">
                 <h2 className="text-base font-semibold text-white tracking-tight">Last records overview</h2>
               ...
               {transactionsToDisplay.length > 0 && (
                  <button onClick={() => { setActiveView('records'); pushNav(); }} className="w-full py-4 text-blue-500 font-semibold text-[10px] uppercase tracking-[0.2em] border-t border-zinc-900/30 hover:bg-zinc-800/20 active:bg-zinc-800/40">Show more</button>
               )}
            </div>
          </div>
        )}
          ...
          <div className="flex flex-col h-full animate-in slide-in-from-right duration-400">
             <div className="p-4 border-b border-zinc-900 bg-[#0e0e10]/80 backdrop-blur-lg sticky top-0 z-20">
               ...
               <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                 {accounts.map(acc => (
                   <button key={acc.id} onClick={() => setSelectedAccountIds(prev => prev.includes(acc.id) ? prev.filter(x => x !== acc.id) : [...prev, acc.id])} className={`flex-shrink-0 px-4 py-2 rounded-2xl text-[10px] font-semibold border transition-all uppercase tracking-wider ${selectedAccountIds.includes(acc.id) ? 'bg-blue-600 border-blue-500 text-white' : 'bg-[#1e1e1e] border-zinc-800 text-zinc-500'}`}>{acc.name}</button>
                 ))}
               </div>
             </div>
             <div className="flex-1 overflow-y-auto p-2 pb-32">
               <div className="bg-[#1e1e1e]/80 rounded-[1.5rem] overflow-hidden border border-zinc-900/50">
        ...
        {activeView === 'categories' && (
          <div className="flex flex-col h-full animate-in slide-in-from-right duration-400">
             ...
             <div className="flex-1 overflow-y-auto p-2 pb-32 space-y-2 no-scrollbar">
               {categories.map(cat => (
                 <button key={cat.id} onClick={() => { setManagingCategory(cat); pushNav(); }} className="w-full bg-[#1e1e1e] border border-zinc-800/50 rounded-[1.25rem] overflow-hidden shadow-lg p-4 flex items-center justify-between active:scale-[0.98] transition-all">
                     <div className="flex flex-col items-start"><span className="font-semibold text-white text-base">{cat.name}</span><span className="text-[9px] font-semibold text-zinc-500 tracking-widest uppercase">{cat.subCategories.length} Sub-items</span></div>
      ...
      <nav className={`fixed bottom-0 left-0 right-0 bg-[#0e0e10]/95 backdrop-blur-2xl border-t border-zinc-900 flex justify-around p-3 pb-10 z-40 safe-bottom transition-transform duration-300 ${selectedRecordIds.length > 0 ? 'translate-y-full' : 'translate-y-0'}`}>
        <button onClick={() => { setActiveView('dashboard'); }} className={`flex flex-col items-center gap-1 transition-all ${activeView === 'dashboard' ? 'text-blue-500 scale-110' : 'text-zinc-500'}`}><LayoutGrid className="w-6 h-6" /><span className="text-[10px] font-semibold uppercase tracking-tighter">Home</span></button>
        <button onClick={() => { setActiveView('records'); pushNav(); }} className={`flex flex-col items-center gap-1 transition-all ${activeView === 'records' ? 'text-blue-500 scale-110' : 'text-zinc-500'}`}><ArrowLeftRight className="w-6 h-6" /><span className="text-[10px] font-semibold uppercase tracking-tighter">Records</span></button>
        <button onClick={() => { setActiveView('categories'); pushNav(); }} className={`flex flex-col items-center gap-1 transition-all ${activeView === 'categories' ? 'text-blue-500 scale-110' : 'text-zinc-500'}`}><PieChart className="w-6 h-6" /><span className="text-[10px] font-semibold uppercase tracking-tighter">Cats</span></button>
      </nav>

      {isAddingTransaction && (
        <div className="fixed inset-0 z-[100] bg-zinc-950 flex flex-col safe-top animate-in slide-in-from-bottom duration-300">
           <div className="bg-blue-600 p-4 shrink-0"><div className="flex items-center justify-between mb-8"><button onClick={() => setIsAddingTransaction(false)}><X className="w-6 h-6 text-white" /></button><button onClick={handleSaveTransaction}><Check className="w-7 h-7 text-white" /></button></div><div className="flex gap-1 bg-blue-700/50 p-1 rounded-2xl border border-blue-400/20">{(['INCOME', 'EXPENSE', 'TRANSFER'] as TransactionType[]).map(type => (<button key={type} onClick={() => { setTxType(type); if(type === 'TRANSFER') setTxCategory('Transfer'); }} className={`flex-1 py-3 text-[10px] font-semibold uppercase tracking-widest rounded-xl transition-all ${txType === type ? 'bg-blue-600 text-white shadow-lg' : 'text-blue-200 opacity-70'}`}>{type}</button>))}</div></div>
           <div className="flex-1 bg-blue-600 flex flex-col items-center justify-center px-8 relative overflow-hidden"><div className="flex items-center gap-3 text-white mb-8 w-full justify-center"><span className="text-4xl opacity-50 shrink-0">{txType === 'INCOME' ? '+' : txType === 'TRANSFER' ? '' : '−'}</span><span className={`font-light tracking-tighter truncate text-center leading-tight transition-all duration-300 ${getAmountFontSize(txAmountStr)}`}>{formatInputAmount(txAmountStr)}</span><span className="text-xl font-semibold opacity-70 shrink-0">{accounts.find(a => a.id === txAccount)?.currency || 'IRR'}</span></div><div className="w-full grid grid-cols-2 gap-3"><button onClick={() => { setShowFromAccountPicker(true); pushNav(); }} className="flex flex-col items-center bg-blue-700/30 p-4 rounded-3xl border border-blue-400/20"><span className="text-[8px] font-semibold text-blue-100 uppercase tracking-widest opacity-60 mb-1">{txType === TransactionType.TRANSFER ? 'From' : 'Account'}</span><span className="text-white font-semibold truncate w-full text-center text-sm">{accounts.find(a => a.id === txAccount)?.name || 'Select'}</span></button>{txType === TransactionType.TRANSFER ? (<button onClick={() => { setShowToAccountPicker(true); pushNav(); }} className="flex flex-col items-center bg-blue-700/30 p-4 rounded-3xl border border-blue-400/20"><span className="text-[8px] font-semibold text-blue-100 uppercase tracking-widest opacity-60 mb-1">To</span><span className="text-white font-semibold truncate w-full text-center text-sm">{accounts.find(a => a.id === txToAccount)?.name || 'Select'}</span></button>) : (<button onClick={() => { setShowCategoryPicker(true); pushNav(); }} className="flex flex-col items-center bg-blue-700/30 p-4 rounded-3xl border border-blue-400/20"><span className="text-[8px] font-semibold text-blue-100 uppercase tracking-widest opacity-60 mb-1">Category</span><span className="text-white font-semibold truncate w-full text-center text-sm">{txSubCategory ? txSubCategory : (txCategory || 'Select')}</span></button>)}</div></div>
           <div className="bg-[#1c1c1f] p-4 shrink-0"><div className="grid grid-cols-4 gap-2">{[7, 8, 9, '/', 4, 5, 6, '*', 1, 2, 3, '-', '.', 0, 'DEL', '+'].map((key, idx) => (<button key={idx} onClick={() => handleKeypadPress(key.toString())} className={`h-14 flex items-center justify-center rounded-2xl text-2xl font-light transition-all active:scale-95 ${typeof key === 'number' || key === '.' ? 'text-zinc-100 bg-[#1e1e1e]' : 'text-zinc-500 bg-[#1e1e1e]/60'}`}>{key === 'DEL' ? <Delete className="w-6 h-6" /> : key === '/' ? '÷' : key}</button>))} <button onClick={() => handleKeypadPress('=')} className="h-14 col-span-4 flex items-center justify-center rounded-2xl bg-blue-600/10 text-blue-500 active:bg-blue-600/20 transition-all"><Equal className="w-6 h-6" /></button></div><div className="grid grid-cols-2 gap-2 mt-4"><div className="bg-[#1e1e1e] rounded-xl px-4 py-3 flex items-center gap-2 border border-zinc-800"><Calendar className="w-4 h-4 text-zinc-600" /><input type="datetime-local" value={txDate} onChange={e => setTxDate(e.target.value)} className="bg-transparent outline-none text-[10px] font-semibold text-zinc-400 w-full" /></div><div className="bg-[#1e1e1e] rounded-xl px-4 py-3 flex items-center gap-2 border border-zinc-800"><Edit2 className="w-4 h-4 text-zinc-600" /><input placeholder="Note..." value={txNote} onChange={e => setTxNote(e.target.value)} className="bg-transparent outline-none text-[10px] font-semibold text-zinc-400 w-full" /></div></div></div>
        </div>
      )}

      {/* Account Manager Modal - Restored */}
      {showAccountManager && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md">
          <div className="w-full max-sm:w-full bg-[#1e1e1e] border border-zinc-800 rounded-[2rem] p-6 space-y-6 shadow-2xl animate-in zoom-in duration-200">
            <div className="flex items-center justify-between"><h3 className="text-lg font-semibold uppercase tracking-tight">Accounts</h3><button onClick={() => setShowAccountManager(false)} className="p-2 bg-[#0e0e10] rounded-full text-zinc-400"><X className="w-5 h-5" /></button></div>
            <div className="space-y-2 max-h-[60vh] overflow-y-auto no-scrollbar">
               {accounts.map(acc => (
                 <div key={acc.id} className="flex items-center justify-between p-4 bg-[#0e0e10] border border-zinc-800 rounded-2xl">
                    <div className="flex items-center gap-3">
            ...
            <button onClick={() => { setIsAddingAccount(true); setShowAccountManager(false); pushNav(); }} className="w-full py-4 bg-blue-600/10 border border-blue-600/30 rounded-2xl font-semibold text-blue-500 hover:bg-blue-600/20 flex items-center justify-center gap-2 active:scale-95 transition-all"><Plus className="w-5 h-5" /> New Wallet</button>
          </div>
        </div>
      )}

      {/* Account Creation/Edit Form Modal - Restored */}
      {(isAddingAccount || editingAccount) && (
        <div className="fixed inset-0 z-[140] flex items-center justify-center p-6 bg-black/95 backdrop-blur-md overflow-y-auto no-scrollbar py-12">
           <div className="w-full max-w-sm bg-[#1e1e1e] border border-zinc-800 rounded-[2rem] p-8 space-y-6 animate-in zoom-in duration-200 shadow-2xl my-auto">
             <div className="flex justify-between items-center">
               <h3 className="text-xl font-semibold uppercase tracking-tight">{isAddingAccount ? 'New Account' : 'Edit Account'}</h3>
             ...
             <div className="space-y-5">
                <div className="space-y-1">
                  <span className="text-[10px] font-semibold text-zinc-600 uppercase tracking-widest ml-4">Account Name</span>
                  <input placeholder="Wallet Name" value={isAddingAccount ? newAccName : tempAccount?.name || ''} onChange={e => isAddingAccount ? setNewAccName(e.target.value) : setTempAccount(prev => prev ? {...prev, name: e.target.value} : null)} className="w-full bg-[#0e0e10] border border-zinc-800 rounded-2xl py-4 px-6 outline-none focus:border-blue-500 font-semibold text-white" />
                </div>
                <div className="space-y-1">
                  <span className="text-[10px] font-semibold text-zinc-600 uppercase tracking-widest ml-4">Initial Balance</span>
                  <input type="number" placeholder="0" value={isAddingAccount ? newAccBalance : tempAccount?.balance || '0'} onChange={e => isAddingAccount ? setNewAccBalance(e.target.value) : setTempAccount(prev => prev ? {...prev, balance: parseFloat(e.target.value) || 0} : null)} className="w-full bg-[#0e0e10] border border-zinc-800 rounded-2xl py-4 px-6 outline-none focus:border-blue-500 font-semibold text-white" />
                </div>
                <div className="space-y-1">
                  <span className="text-[10px] font-semibold text-zinc-600 uppercase tracking-widest ml-4">Account Type</span>
                ...
                <div className="space-y-3">
                  <span className="text-[10px] font-semibold text-zinc-600 uppercase tracking-widest ml-4">Accent Color</span>
             ...
             <div className="flex flex-col gap-3 pt-2">
                <button onClick={isAddingAccount ? handleAddAccount : handleUpdateAccount} className="w-full py-5 bg-blue-600 rounded-2xl font-semibold text-white shadow-lg active:scale-95 transition-all uppercase tracking-widest">{isAddingAccount ? 'Create Account' : 'Save Changes'}</button>
                {editingAccount && <button onClick={() => handleDeleteAccount(editingAccount.id)} className="w-full py-4 text-rose-500 font-semibold text-xs bg-rose-500/10 rounded-2xl border border-rose-500/20 active:bg-rose-500/20 transition-all uppercase tracking-widest">Delete Wallet</button>}
             </div>
           </div>
        </div>
      )}

      {/* Account Selection Picker (From) - Restored */}
      {showFromAccountPicker && (
        <div className="fixed inset-0 z-[200] bg-[#0e0e10] flex flex-col p-6 safe-top animate-in fade-in zoom-in duration-200">
           <div className="flex items-center justify-between mb-8"><h3 className="text-xl font-semibold text-white uppercase tracking-tight">Select Account</h3><button onClick={() => setShowFromAccountPicker(false)} className="p-2 bg-[#1e1e1e] rounded-full text-zinc-400"><X className="w-6 h-6" /></button></div>
           <div className="flex-1 overflow-y-auto no-scrollbar space-y-3 pb-20">{accounts.map(acc => (<button key={acc.id} onClick={() => { setTxAccount(acc.id); setShowFromAccountPicker(false); }} className="w-full flex items-center justify-between p-5 bg-[#1e1e1e] border border-zinc-800 rounded-[2rem] active:scale-[0.98] transition-all"><div className="flex items-center gap-4"><div style={{ color: acc.color }} className="opacity-80"><AccountIcon type={acc.type} className="w-6 h-6" /></div><span className="font-semibold text-xl text-zinc-100">{acc.name}</span></div><span className="text-zinc-500 font-semibold tracking-widest text-sm">{acc.currency}</span></button>))}</div>
        </div>
      )}

      {/* Account Selection Picker (To) - Restored */}
      {showToAccountPicker && (
        <div className="fixed inset-0 z-[200] bg-[#0e0e10] flex flex-col p-6 safe-top animate-in fade-in zoom-in duration-200">
           <div className="flex items-center justify-between mb-8"><h3 className="text-xl font-semibold text-white uppercase tracking-tight">To Destination</h3><button onClick={() => setShowToAccountPicker(false)} className="p-2 bg-[#1e1e1e] rounded-full text-zinc-400"><X className="w-6 h-6" /></button></div>
      ...
      {/* Category Selection Picker - Restored */}
      {showCategoryPicker && (
        <div className="fixed inset-0 z-[200] bg-[#0e0e10] flex flex-col safe-top animate-in slide-in-from-bottom duration-300">
           <header className="p-6 flex items-center justify-between border-b border-zinc-900">
             <div className="flex items-center gap-4">
               ...
               <h3 className="text-xl font-semibold uppercase tracking-tight">{selectedMainCategory ? selectedMainCategory.name : 'Select Category'}</h3>
             </div>
             <div className="flex items-center gap-3">
               {!selectedMainCategory && (
                ...
                filteredFlatCategories.map((item, idx) => (
                  <button key={idx} onClick={() => { setTxCategory(item.main); setTxSubCategory(item.sub || ''); setShowCategoryPicker(false); setCategorySearch(''); setSelectedMainCategory(null); setIsSearchingCategory(false); }} className="w-full flex items-center justify-between p-5 bg-[#1e1e1e] border border-zinc-800 rounded-[2rem] active:scale-[0.98] transition-all">
                    <div className="flex items-center gap-4">
                      ...
                      <span className="font-semibold text-xl text-zinc-100">{item.display}</span>
                    </div>
                  </button>
                ))
              ) : selectedMainCategory ? (
                <>
                  <button onClick={() => { setTxCategory(selectedMainCategory.name); setTxSubCategory(''); setShowCategoryPicker(false); setSelectedMainCategory(null); }} className="w-full flex items-center justify-between p-5 bg-blue-600/10 border border-blue-500/30 rounded-[2rem] active:scale-[0.98] transition-all mb-4">
                    <div className="flex items-center gap-4">
                      ...
                      <span className="font-semibold text-xl text-blue-500">Use "{selectedMainCategory.name}"</span>
                    </div>
                  </button>
                  <span className="text-[10px] font-semibold text-zinc-600 uppercase tracking-widest block mb-3 ml-4">Sub Categories</span>
                  {selectedMainCategory.subCategories.map((sub, idx) => (
                    <button key={idx} onClick={() => { setTxCategory(selectedMainCategory.name); setTxSubCategory(sub); setShowCategoryPicker(false); setSelectedMainCategory(null); }} className="w-full flex items-center justify-between p-5 bg-[#1e1e1e] border border-zinc-800 rounded-[2rem] active:scale-[0.98] transition-all">
                      <div className="flex items-center gap-4">
                        ...
                        <span className="font-semibold text-xl text-zinc-100">{sub}</span>
                      </div>
                    </button>
                  ))}
                </>
                ...
                categories.map(cat => (
                  <button key={cat.id} onClick={() => setSelectedMainCategory(cat)} className="w-full flex items-center justify-between p-5 bg-[#1e1e1e] border border-zinc-800 rounded-[2rem] active:scale-[0.98] transition-all group">
                    <div className="flex items-center gap-4">
                      ...
                      <span className="font-semibold text-xl text-zinc-100 group-active:text-blue-500 transition-colors">{cat.name}</span>
                    </div>
                    <span className="text-zinc-500 font-semibold tracking-widest text-sm uppercase">{cat.subCategories.length} Items</span>
                  </button>
                ))
              )}
           </div>
      ...
      {/* Category Detail/Sub-Category Management - Restored */}
      {managingCategory && (
        <div className="fixed inset-0 z-[160] bg-[#0e0e10] flex flex-col p-6 safe-top animate-in slide-in-from-bottom duration-300">
           <div className="flex items-center justify-between mb-8">
             ...
             <h3 className="text-xl font-semibold uppercase tracking-tight">Manage Details</h3>
           ...
           <div className="bg-[#1e1e1e] p-6 h-36 rounded-[2rem] border border-zinc-800 mb-8 flex flex-col justify-center overflow-hidden">
             <span className="text-[10px] font-semibold text-zinc-600 uppercase tracking-widest block mb-2 px-1 text-left">Category Name</span>
             <div className="flex items-center justify-start w-full relative h-12">
                {isEditingCategoryName ? (
                  <>
                    <input autoFocus value={editingCategoryValue} onChange={e => setEditingCategoryValue(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && updateCategoryName(managingCategory.id, editingCategoryValue)} className="bg-[#0e0e10] border-b border-blue-500 outline-none text-xl font-semibold text-white w-full pr-14 py-2 px-4 rounded text-left" />
                  ...
                  <>
                    <span className="text-2xl font-semibold text-white truncate text-left pr-14 leading-tight">{managingCategory.name}</span>
           ...
           <div className="flex-1 overflow-y-auto no-scrollbar pb-20 px-1">
              <div className="flex items-center justify-between mb-4 px-2"><span className="text-xs font-semibold text-zinc-500 uppercase tracking-widest">Sub-categories</span><button onClick={() => setIsAddingSub(true)} className="text-blue-500 font-semibold flex items-center gap-1 bg-blue-500/10 px-4 py-1.5 rounded-full text-sm active:scale-95 transition-all"><Plus className="w-4 h-4" /> Add New</button></div>
              <div className="space-y-2">
                 {isAddingSub && (
                   <div className="p-4 bg-[#1e1e1e] border-2 border-blue-500/50 rounded-2xl animate-in slide-in-from-top duration-200 flex items-center gap-4">
                     <input autoFocus placeholder="New..." value={newSubValue} onChange={e => setNewSubValue(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddSubCategory(managingCategory.id, newSubValue)} className="bg-transparent outline-none font-semibold text-white flex-1" />
