...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      // Logic to close top-most active modal/view
  ...
  }, [showCategoryPicker, showFromAccountPicker, showToAccountPicker, isAddingTransaction, isAddingAccount, editingAccount, showAccountManager, managingCategory, activeView]);

  // Helper to push state when a modal opens
  ...
  // Fix: Added handleCloneTransaction
  ...
  // Fix: Added handleAddSubCategory
  ...
  // Fix: Added handleDeleteSubCategory
  ...
  return (
    <div className="max-w-md mx-auto h-[100dvh] bg-[#0e0e10] flex flex-col relative overflow-hidden select-none">
      ...
      {showCategoryPicker && (
        <div className="fixed inset-0 z-[200] bg-[#0e0e10] flex flex-col safe-top animate-in slide-in-from-bottom duration-300">
           <header className="p-6 flex items-center justify-between border-b border-zinc-900">
             ...
             <button onClick={() => { setShowCategoryPicker(false); setSelectedMainCategory(null); setCategorySearch(''); }} className="p-2 bg-[#1e1e1e] rounded-full text-zinc-400"><X className="w-6 h-6" /></button>
           </header>

           <div className="p-6 pb-0">
              <div className="relative">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-zinc-500" />
                <input placeholder="Search records..." value={categorySearch} onChange={e => setCategorySearch(e.target.value)} className="w-full bg-[#1e1e1e] border border-zinc-800 rounded-2xl py-4 pl-12 pr-6 outline-none focus:border-blue-600 font-medium text-white" />
              </div>
           </div>
           ...
           <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-2">
              {categorySearch ? (
                filteredFlatCategories.map((item, idx) => (
                  <button key={idx} onClick={() => { setTxCategory(item.main); setTxSubCategory(item.sub || ''); setShowCategoryPicker(false); setCategorySearch(''); setSelectedMainCategory(null); }} className="w-full text-left p-5 rounded-[1.5rem] bg-[#1e1e1e]/60 border border-zinc-800 flex items-center justify-between active:scale-[0.98] transition-all"><span className={`font-bold ${item.sub ? 'text-zinc-400 text-sm pl-4' : 'text-zinc-100 text-lg'}`}>{item.display}</span><ChevronRight className="w-4 h-4 text-zinc-700" /></button>
                ))
              ) : selectedMainCategory ? (
                <>
                  <button onClick={() => { setTxCategory(selectedMainCategory.name); setTxSubCategory(''); setShowCategoryPicker(false); setSelectedMainCategory(null); }} className="w-full text-left p-5 rounded-[1.5rem] bg-blue-600/10 border border-blue-500/30 flex items-center justify-between mb-4"><span className="font-bold text-blue-400 text-lg">Use "{selectedMainCategory.name}" main</span><Check className="w-5 h-5 text-blue-500" /></button>
                  <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest block mb-4 ml-4">Sub Categories</span>
                  {selectedMainCategory.subCategories.map((sub, idx) => (
                    <button key={idx} onClick={() => { setTxCategory(selectedMainCategory.name); setTxSubCategory(sub); setShowCategoryPicker(false); setSelectedMainCategory(null); }} className="w-full text-left p-5 rounded-[1.5rem] bg-[#1e1e1e] border border-zinc-800 flex items-center justify-between active:scale-[0.98] transition-all mb-2"><span className="font-bold text-zinc-100 text-lg">{sub}</span><ChevronRight className="w-4 h-4 text-zinc-700" /></button>
                  ))}
                </>
              ) : (
                categories.map(cat => (
                  <button key={cat.id} onClick={() => setSelectedMainCategory(cat)} className="w-full text-left p-6 rounded-[2rem] bg-[#1e1e1e] border border-zinc-800/50 flex items-center justify-between active:scale-[0.98] transition-all mb-3 shadow-lg group"><div className="flex flex-col"><span className="font-bold text-zinc-100 text-lg group-active:text-blue-400 transition-colors">{cat.name}</span><span className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">{cat.subCategories.length} options</span></div><ChevronRight className="w-5 h-5 text-zinc-800 group-active:text-blue-500 transition-colors" /></button>
                ))
              )}
           </div>
        </div>
      ...
      {/* Other Modals... (Simplified for logic) */}
      {isAddingTransaction && (
      ...
      {(isAddingAccount || editingAccount) && (
        <div className="fixed inset-0 z-[140] flex items-center justify-center p-6 bg-black/95 backdrop-blur-md overflow-y-auto no-scrollbar py-12">
           <div className="w-full max-w-sm bg-[#1e1e1e] border border-zinc-800 rounded-[2.5rem] p-8 space-y-6 animate-in zoom-in duration-200 shadow-2xl my-auto">
             <div className="flex justify-between items-center"><h3 className="text-xl font-bold uppercase tracking-tight">{isAddingAccount ? 'New Account' : 'Edit Account'}</h3><button onClick={() => {setIsAddingAccount(false); setEditingAccount(null); setTempAccount(null);}}><X className="w-6 h-6 text-zinc-500" /></button></div>
             <div className="space-y-5">
                <div className="space-y-1"><span className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest ml-4">Name</span><input placeholder="Wallet Name" value={isAddingAccount ? newAccName : tempAccount?.name || ''} onChange={e => isAddingAccount ? setNewAccName(e.target.value) : setTempAccount(prev => prev ? {...prev, name: e.target.value} : null)} className="w-full bg-[#0e0e10] border border-zinc-800 rounded-2xl py-4 px-6 outline-none focus:border-blue-500 font-bold text-white" /></div>
                <div className="space-y-1"><span className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest ml-4">Balance</span><input type="number" value={isAddingAccount ? newAccBalance : tempAccount?.balance || '0'} onChange={e => isAddingAccount ? setNewAccBalance(e.target.value) : setTempAccount(prev => prev ? {...prev, balance: parseFloat(e.target.value) || 0} : null)} className="w-full bg-[#0e0e10] border border-zinc-800 rounded-2xl py-4 px-6 outline-none focus:border-blue-500 font-bold text-white" /></div>
                
                <div className="space-y-3">
                  <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest ml-4">Color</span>
                  <div className="bg-[#0e0e10] p-6 rounded-3xl border border-zinc-800">
                    <div className="grid grid-cols-4 gap-4 mb-6">
                      {PRESET_COLORS.map(color => (
                        <button key={color} onClick={() => isAddingAccount ? setNewAccColor(color) : setTempAccount(prev => prev ? {...prev, color} : null)} style={{ backgroundColor: color }} className={`w-full aspect-square rounded-full border-2 transition-all ${(isAddingAccount ? newAccColor === color : tempAccount?.color === color) ? 'border-white scale-125 ring-2 ring-white/20' : 'border-transparent opacity-40 hover:opacity-100'}`} />
                      ))}
                    </div>
                    <div className="flex items-center justify-between">
                       <span className="text-[11px] font-mono text-zinc-500 tracking-tighter uppercase">{isAddingAccount ? newAccColor : tempAccount?.color}</span>
                       <div className="relative overflow-hidden bg-zinc-800 rounded-xl px-4 py-2 flex items-center gap-2 active:bg-zinc-700 transition-colors">
                          <Palette className="w-3.5 h-3.5 text-zinc-400" />
                          <span className="text-[10px] font-bold uppercase text-zinc-300">Pick</span>
             ...
             <div className="flex flex-col gap-2 pt-2"><button onClick={isAddingAccount ? handleAddAccount : handleUpdateAccount} className="w-full py-5 bg-blue-600 rounded-2xl font-black text-white shadow-lg active:scale-95 transition-all uppercase tracking-widest">{isAddingAccount ? 'Create' : 'Save'}</button>{editingAccount && <button onClick={() => handleDeleteAccount(editingAccount.id)} className="w-full py-4 text-rose-500 font-bold text-sm bg-rose-500/10 rounded-2xl border border-rose-500/20 active:bg-rose-500/20 transition-all uppercase tracking-widest">Delete</button>}</div>
           </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md">
          <div className="w-full max-sm:w-full bg-[#1e1e1e] border border-zinc-800 rounded-[2.5rem] p-6 space-y-6 shadow-2xl">
            ...
            <div className="space-y-2 max-h-[60vh] overflow-y-auto no-scrollbar">
               {accounts.map(acc => (<div key={acc.id} className="flex items-center justify-between p-4 bg-[#0e0e10] border border-zinc-800 rounded-2xl"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${acc.color}20`, color: acc.color }}><AccountIcon type={acc.type} className="w-5 h-5" /></div><span className="font-bold">{acc.name}</span></div><button onClick={() => handleStartEditAccount(acc)} className="p-2 text-blue-400 bg-blue-400/10 rounded-xl"><Edit2 className="w-4 h-4" /></button></div>))}
            </div>
          </div>
        </div>
      )}
      ...
      {showFromAccountPicker && (
        <div className="fixed inset-0 z-[200] bg-[#0e0e10] flex flex-col p-6 safe-top animate-in fade-in zoom-in duration-200">
           ...
           <div className="flex-1 overflow-y-auto no-scrollbar space-y-3 pb-20">{accounts.map(acc => (<button key={acc.id} onClick={() => { setTxAccount(acc.id); setShowFromAccountPicker(false); }} className="w-full flex items-center justify-between p-5 bg-[#1e1e1e] border border-zinc-800 rounded-[1.8rem] active:scale-[0.98] transition-all"><div className="flex items-center gap-4"><div style={{ color: acc.color }} className="opacity-80"><AccountIcon type={acc.type} className="w-5 h-5" /></div><span className="font-bold text-lg text-zinc-100">{acc.name}</span></div><span className="text-zinc-500 font-bold tracking-widest text-sm">{acc.currency}</span></button>))}</div>
        </div>
      )}

      {showToAccountPicker && (
        <div className="fixed inset-0 z-[200] bg-[#0e0e10] flex flex-col p-6 safe-top animate-in fade-in zoom-in duration-200">
           <div className="flex items-center justify-between mb-8"><h3 className="text-xl font-bold text-white uppercase tracking-tight">Select Destination</h3><button onClick={() => setShowToAccountPicker(false)} className="p-2 bg-[#1e1e1e] rounded-full text-zinc-400"><X className="w-6 h-6" /></button></div>
           <div className="flex-1 overflow-y-auto no-scrollbar space-y-3 pb-20">{accounts.filter(a => a.id !== txAccount).map(acc => (<button key={acc.id} onClick={() => { setTxToAccount(acc.id); setShowToAccountPicker(false); }} className="w-full flex items-center justify-between p-5 bg-[#1e1e1e] border border-zinc-800 rounded-[1.8rem] active:scale-[0.98] transition-all"><div className="flex items-center gap-4"><div style={{ color: acc.color }} className="opacity-80"><AccountIcon type={acc.type} className="w-5 h-5" /></div><span className="font-bold text-lg text-zinc-100">{acc.name}</span></div><span className="text-zinc-500 font-bold tracking-widest text-sm">{acc.currency}</span></button>))}</div>
        </div>
      )}
    </div>
  );
